...
{
    struct stat buffer;
    return wstat(file, &buffer) ? 0 : buffer.st_ino;
}
...
int IsFile(const char *file) {
    struct stat buf;
    return (!wstat(file, &buf) && S_ISREG(buf.st_mode)) ? 0 : -1;
}
...
int IsSocket(const char * file) {
    struct stat buf;
    return (!wstat(file, &buf) && S_ISSOCK(buf.st_mode)) ? 0 : -1;
}
...
off_t FileSize(const char * path) {
    struct stat buf;
    return wstat(path, &buf) ? -1 : buf.st_size;
}
...
char *w_homedir(char *arg) {
    ...
    if ((wstat(buff, &buff_stat) < 0) || !S_ISDIR(buff_stat.st_mode)) {
        os_free(buff);
